...
/**
 * Experimental Soy to Go Src compiler.
 *
 * <p> Warning: The Go Src backend is experimental (repetitive, untested, undocumented).
 *
 * @author Kai Huang
 */
public class SoyToGoSrcCompilerExperimental {

  private SoyToGoSrcCompilerExperimental() {}


  /** The string to prepend to the usage message. */
  private static final String USAGE_PREFIX =
      "Usage:\n" +
      "go com.google.template.soy.SoyToGoSrcCompilerExperimental  \\\n" +
  ...
  @Option(name = "--codeStyle",
          usage = "The code style to use when generating Go code ('stringbuilder' or 'concat').")
  ...
  @Option(name = "--compileTimeGlobalsFile",
          usage = "The path to a file containing the mappings for global names to be substituted" +
                  ...
                  " from Go, consider using the utility" +
  ...
  public static void main(String[] args) throws IOException, SoySyntaxException {
    (new SoyToGoSrcCompilerExperimental()).execMain(args);
  }
  ...
  private void execMain(String[] args) throws IOException, SoySyntaxException {
    ...
    // Create SoyGoSrcOptions.
    SoyGoSrcOptions goSrcOptions = new SoyGoSrcOptions();
    goSrcOptions.setCodeStyle(codeStyle);
    goSrcOptions.setBidiGlobalDir(bidiGlobalDir);
    ...
    // Compile.
    List<Pair<String, String>> generatedCode = sfs.compileToGoSrc(goSrcOptions, msgBundle);
    
    // Output.
    if (outputPath.length() > 0) {
      for(Pair<String, String> namespaceToCode : generatedCode) {
